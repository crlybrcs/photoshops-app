...
router.post('/test', (req, res) => {
	...
	axios
		.get(`https://api.zinc.io/v1/search?query=${keywords}&page=1&retailer=amazon`, {
...
router.get('/myfavorites', (req, res) => {
	User.findById(req.user._id).populate('products').then((user) => {
	...
	Product.create({ ...req.body }).then(
		...
		(response) => {
			...
			User.findByIdAndUpdate(req.user._id, {
				...
				console.log(responseFromDB.products[responseFromDB.products.length - 1]);
...
router.put('/favorite', (req, res) => {
	...
	User.findById(req.user._id)
		...
		.then((userInfo) => {
			...
			const [ productToDelete ] = userInfo.products.filter((el) => el.product_id === product_id);
